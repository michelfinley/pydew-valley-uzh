...
class SpriteChunk:
    ...
    def __init__(self, pos: tuple[int, int], size: tuple[float, float], on_sprite_exit_chunk: Callable[[Sprite], None]):
        self._pos = pos
        self._size = size
    ...
    def sprite_in_chunk(self, sprite: Sprite) -> bool:
        return (int(sprite.rect.left / self._size[0]) == self._pos[0] and
                int(sprite.rect.top / self._size[1]) == self._pos[1])
...
class SpriteLayer:
    ...
    def get_sprite_chunk(self, sprite: Sprite):
        pos = int(sprite.rect.left / self._chunk_size[0]), int(sprite.rect.top / self._chunk_size[1])
