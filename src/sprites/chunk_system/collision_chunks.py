...
class CollisionManager(SpriteManager):
    ...
    def __init__(self):
        self._containers = DefaultCallableDict(
            lambda x: CollisionChunk(x, (COLLISION_CHUNK_W, COLLISION_CHUNK_H), self.on_sprite_exit_chunk)
        )
    ...
    def update_sprite(self, sprite: MovingSprite) -> bool:
        ...
        chunks = [self.get_chunk((i, j)) for j in range(center_chunk[1] - 1, center_chunk[1] + 2) for i in range(center_chunk[0] - 1, center_chunk[0] + 2)]
